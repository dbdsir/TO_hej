...
def deal_local(dec_delay, task_cal, t_vehicle_remain_cal, t_vehicle_power_cal):
    reward_delay = dec_delay + ((task_cal * 10 ** 4) / (t_vehicle_remain_cal * 10 ** 6))
    reward_consumption = reward_delay * t_vehicle_power_cal
    return reward_delay, reward_consumption


class Mec_Env:
    action_space_tup = ('bool_offload', 'mec_or_cc', 'offload_number', 'channel_number',
                        'Distribution_ratio')
    action_dim = 4
    state_dim = 14
    action_bound = [-1, 1]
    ...
    def step(self, state, action: np.ndarray) -> (np.ndarray, float, bool, dict):
        reward, done, info_dict = self.step_real(state, action)
        return float(reward), done, info_dict

    def step_real(self, state, action: ActType) -> (ObsType, float, bool, dict):
        done = False
        sign_time_out = False
        ...
        if node_id == 0:
            ...
            v_trans = self.bandwidth_m * np.log2(
                1 + self.channel_gain * signal_launch_p / (
                        self.noise_t + 5 * self.channel_gain))  # 5Gbps
            ...
            if node_id != 4:
                ...
                t_attach = abs(
                    ...
                    1 + self.pass_loss_ratio) / 20  # 0.0224
        ...
        if node_id != 0 and node_id != 4:
            self.gnb_info['gnb_load_rate'][node_id - 1] = x
